...
def minimax(board, depth, max_player):
    ...
    if max_player:
        ...
        for col in valid_locations:
            ...
            #new_score = max(value, minimax(col, depth - 1, False))
        ...
        for col in valid_locations:
            ...
            b_copy = board.copy()
            drop_piece(b_copy, row, col, AI_PIECE)
            #new_score = min(value, minimax(col, depth - 1, True))
...
def evaluate_window(window, piece):
    ...
    if window.count(piece) == 4:
        score += 100
    if window.count(piece) == WINDOW_LENGTH -1 and window.count(EMPTY) == 1:
        score += 5 #10
    if window.count(piece) == WINDOW_LENGTH -2 and window.count(EMPTY) == 2:
        score += 2 #5
